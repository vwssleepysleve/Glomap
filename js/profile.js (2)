...
const userIdElement = document.getElementById("user-id"); // User ID element (App unique username)
...
// Get unique userID from URL parameters
...
if (userID) {
  ...
  getDoc(userRef).then((docSnap) => {
    if (docSnap.exists()) {
      ...
      username.textContent = data.userName || "No Username"; // Show the unique username
      userIdElement.textContent = `@${data.userName}`; // Display the user's unique app ID
